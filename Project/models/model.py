import logging
from utils.general import batches, get_random_samples, find_best_span, save_graphs
...
class Model(metaclass=ABCMeta):
    ...
    def run_epoch(self, session, train, val, log):
        ...
        self.results.save("batch_size", self.config.batch_size)
        ...
        for i, train_batch in enumerate(
            ...
            if i % self.config.eval_num == 0 or i == num_batches:
                ...
                self.results.save("f1_train", f1_train)
                self.results.save("EM_train", EM_train)
                self.results.save("f1_val", f1_val)
                self.results.save("EM_val", EM_val)
                batches_trained = 1 if self.results.is_empty("batch_indices") \
                else self.results.get("batch_indices")[-1] + min(i + 1, self.config.eval_num)

                self.results.save("batch_indices", batches_trained)

                save_graphs(self.results.data, path=self.config.train_dir)
                if f1_val > best_f1:

...
def processing_logit_softmax(logits, mask):
    ...
    mask_value = tf.multiply(new_mask, tf.constant(-2e7))
    masked_logits = tf.where(mask, logits, mask_value)
    return masked_logits


def processing_logit(context, max_context_length):
...
def BiLSTM(inputs, masks, size, initial_state_fw=None, initial_state_bw=None, dropout=1.0, reuse=False):
    cell_fw = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.BasicLSTMCell(size, reuse=reuse), input_keep_prob=dropout)
    cell_bw = tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.BasicLSTMCell(size, reuse=reuse), input_keep_prob=dropout)
